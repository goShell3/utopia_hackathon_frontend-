...
import { Plus, Settings, Zap, ExternalLink, RefreshCcw, ShieldCheck, Layers, MessageCircle, BarChart3, Globe } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4 scrollbar-hide overflow-x-auto pb-4">
        {['All Nodes', 'PMS (2)', 'Marketing (1)', 'SMS Gateway (1)', 'Analytics (0)'].map((label, i) => (
          <button key={label} className={cn("px-6 py-2 whitespace-nowrap text-[10px] technical-label border-[1.5px] uppercase transition-all",
            i === 0 ? "bg-black text-white border-black" : "bg-white text-neutral-400 border-neutral-200 hover:border-black hover:text-black"
          )}>{label}</button>
        ))}
      </div>
      ...
      <div className="industrial-card p-6 bg-black text-white relative overflow-hidden">
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-utopia flex items-center justify-center rounded-[1px]"><ShieldCheck size={28} /></div>
            <div>
