...
import { 
  Search, Filter, FileDown, Plus, MoreVertical, MessageSquare, Star, ChevronDown
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="primary" size="md" icon={Plus}>Manual Entry</Button>
        </div>
      </div>
      ...
      <div className="industrial-card p-4 flex items-center gap-6">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" icon={Filter} className="bg-white px-3">Source: All</Button>
          <Button variant="outline" size="sm" icon={Star} className="bg-white px-3">Score: 80+</Button>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-neutral-100">
            {leadsData.map((lead) => (
              <tr key={lead.id} className="hover:bg-neutral-50 transition-colors group">
                ...
                <td className="p-4"><span className="text-xs font-mono technical-label text-black">{lead.phone}</span></td>
                <td className="p-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 px-2 py-1 bg-neutral-100 rounded-[1px]">{lead.source}</span>
                </td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                ...
                <td className="p-4 text-right">
                  <div className="flex flex-col items-end">
                    <span className={cn("text-sm font-black italic", lead.ltv > 1000 ? "text-utopia" : "text-black")}>
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-2 hover:bg-neutral-200 rounded-[1px] transition-colors"><MessageSquare size={14} className="text-black" /></button>
                    <button className="p-2 hover:bg-neutral-200 rounded-[1px] transition-colors"><MoreVertical size={14} className="text-neutral-400" /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className="flex items-center justify-between p-4 bg-white border border-neutral-200 -mt-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            {[1,2,3,4].map(n => (
              <button key={n} className={cn("w-6 h-6 flex items-center justify-center text-[10px] font-bold technical-label hover:bg-neutral-100",
                n === 1 ? "bg-black text-white" : "text-neutral-500"
              )}>{n}</button>
            ))}
          </div>
        </div>
        <Button variant="outline" size="sm" icon={ChevronDown} className="bg-white technical-label text-[10px] px-4">Rows: 10</Button>
      </div>
    </div>
  );
}
