...
import { MessageSquare, Send, Search, MoreVertical, CheckCheck, Plus, Zap, Filter, Paperclip, Smile, Sparkles } from 'lucide-react';
    ...
    <div className="h-[calc(100vh-140px)] flex gap-6 animate-in fade-in duration-500">
      <div className="w-80 flex flex-col industrial-card p-0 h-full overflow-hidden">
        <div className="p-6 border-b border-neutral-200">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search conversations..." className="w-full bg-neutral-100 border border-neutral-200 py-2 pl-9 pr-3 text-[10px] technical-label focus:outline-none focus:ring-1 focus:ring-utopia/30" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto divide-y divide-neutral-100 italic">
          {conversations.map((conv) => (
            <div key={conv.id} onClick={() => setActiveConv(conv.id)} className={cn("p-4 cursor-pointer transition-all hover:bg-neutral-50", activeConv === conv.id ? "bg-black text-white" : "text-black")}>
              <div className="flex justify-between items-start mb-1">
                <span className="text-xs font-black italic tracking-tight">{conv.name}</span>
                <span className={cn("text-[8px] technical-label", activeConv === conv.id ? "text-neutral-500" : "text-neutral-400")}>{conv.time}</span>
              </div>
              <p className={cn("text-[10px] truncate max-w-[180px]", activeConv === conv.id ? "text-neutral-400" : "text-neutral-500")}>{conv.lastMsg}</p>
              <div className="flex items-center justify-between mt-2">
                <div className="flex items-center gap-1">
                  {conv.status === 'Responded' ? <div className="w-1.5 h-1.5 rounded-full bg-utopia" /> : <CheckCheck size={10} className={activeConv === conv.id ? "text-utopia" : "text-neutral-400"} />}
                  <span className="text-[8px] technical-label uppercase">{conv.status}</span>
                </div>
                {conv.unread && activeConv !== conv.id && <span className="w-2 h-2 bg-utopia rounded-full" />}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col industrial-card p-0 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-neutral-50/30 italic">
          {messageHistory.map((msg) => (
            <div key={msg.id} className={cn("flex flex-col max-w-[70%] space-y-1", msg.sender === 'System' ? "ml-auto items-end" : "items-start")}>
              <div className={cn("p-4 shadow-sm", msg.sender === 'System' ? "bg-black text-white rounded-l-2xl rounded-tr-2xl" : "bg-white text-black border border-neutral-200 rounded-r-2xl rounded-tl-2xl")}>
        ...
        <div className="p-6 bg-white border-t border-neutral-200">
          <div className="bg-neutral-100 border border-neutral-200 rounded-xl p-2 focus-within:ring-2 focus-within:ring-utopia/10 transition-all flex items-end gap-2">
            ...
            <textarea placeholder="Protocol: Enter secure SMS content..." className="flex-1 bg-transparent border-none outline-none text-xs font-bold italic resize-none py-2 min-h-[40px] max-h-32" />
            <button className="bg-utopia text-white p-3 rounded-lg hover:bg-utopia/90 transition-all shadow-lg shadow-utopia/20 -translate-y-0.5"><Send size={18} /></button>
          </div>
      ...
      <div className="w-80 flex flex-col space-y-6">
        <div className="industrial-card p-6 bg-utopia text-white border-none shadow-xl shadow-utopia/20">
          <div className="flex items-center gap-3 mb-6"><Zap size={24} /><h3 className="display-header text-xl italic uppercase font-black">Bulk Signal</h3></div>
          ...
          <Button variant="outline" size="md" className="w-full bg-white text-utopia border-none hover:bg-white/90" icon={Plus}>Broadcast Manually</Button>
        </div>
        <div className="industrial-card p-6 flex-1 flex flex-col">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto mb-6 italic">
            {[{ label: 'High LTV VIPs', count: '1,240' }, { label: 'Recent Checkouts', count: '842', active: true }, { label: 'Churn Risk (30d)', count: '4,923' }, { label: 'Meta Ad Leads', count: '212' }].map(s => (
              <div key={s.label} className={cn("p-3 border flex items-center justify-between cursor-pointer transition-all hover:scale-[1.02]", s.active ? "bg-black border-black text-white" : "bg-white border-neutral-100 text-black hover:border-black")}>
                <span className="text-[10px] font-black italic tracking-widest uppercase">{s.label}</span>
                <span className={cn("text-[9px] technical-label px-1.5 py-0.5 rounded-[1px]", s.active ? "bg-white text-black" : "bg-neutral-100 text-neutral-500")}>{s.count}</span>
              </div>
            ))}
          </div>
          <Button variant="primary" size="md" className="w-full" icon={Filter}>Apply Segment</Button>
        </div>
      </div>
    </div>
  );
