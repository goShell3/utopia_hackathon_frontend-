...
import { Zap, Plus, Send, Calendar, BarChart3, BellRing, MoreVertical, ChevronRight, Sparkles, MessageSquare } from 'lucide-react';
        ...
        {campaignData.map((campaign) => (
          <div key={campaign.id} onClick={() => setSelectedId(campaign.id)}
            className={cn("industrial-card p-6 flex flex-col justify-between group cursor-pointer transition-all hover:scale-[1.02]",
              selectedId === campaign.id ? "ring-2 ring-utopia border-transparent shadow-xl" : ""
            )}>
            <div className="flex items-start justify-between mb-4">
              <div className={cn("p-2 rounded-full", campaign.status === 'Active' ? "bg-emerald-50 text-emerald-600" : "bg-neutral-100 text-neutral-400")}>
                <Zap size={16} />
              </div>
              <button className="text-neutral-300 hover:text-black"><MoreVertical size={16} /></button>
            </div>
            <div className="mb-6">
        ...
        <div className="h-[400px] border-2 border-dashed border-neutral-200 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden bg-white/50 backdrop-blur-sm">
          {!selectedId ? (
            <div className="text-center p-12">
              ...
              <p className="technical-label text-neutral-300 uppercase">Select a campaign to visualize its logic flow</p>
            </div>
          ) : (
            <>
