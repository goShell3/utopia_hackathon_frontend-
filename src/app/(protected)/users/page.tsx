...
import { Plus, Search, ShieldCheck, Lock, Trash2, MoreVertical, Check } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" size="md" icon={Lock} className="bg-white">Audit Logs</Button>
          <Button variant="primary" size="md" icon={Plus}>Create User</Button>
        </div>
      </div>
      ...
      <div className="industrial-card p-4 flex items-center gap-6">
        <div className="flex-1 relative group">
          ...
          <input type="text" placeholder="Search identities by name or email..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <tbody className="divide-y divide-neutral-100">
              {usersData.map((user) => (
                <tr key={user.id} className="hover:bg-neutral-50 transition-colors group">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-black text-white flex items-center justify-center rounded-[1px] font-black italic shadow-md">{user.name.split(' ').map(n => n[0]).join('')}</div>
                  ...
                  <td className="p-4">
                    <div className={cn("w-fit px-3 py-1 text-[10px] technical-label font-bold border rounded-[1px]",
                  ...
                  <td className="p-4 text-right"><button className="text-neutral-300 hover:text-black group-hover:opacity-100 opacity-0 transition-all"><MoreVertical size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="industrial-card p-6 flex flex-col space-y-8 bg-neutral-50/50">
          ...
          <div className="space-y-6">
            {[{ label: 'Modify Lead Database', enabled: true }, { label: 'Execute SMS Campaigns', enabled: true }, { label: 'Export CSV/Analytics', enabled: true }, { label: 'Integration Protocol Access', enabled: false }, { label: 'System Identity Management', enabled: false }].map(p => (
              <div key={p.label} className="flex items-center justify-between p-3 bg-white border border-neutral-100 rounded-[1px] hover:border-black transition-all group cursor-pointer">
                <span className="text-xs font-black italic tracking-tighter uppercase">{p.label}</span>
                <div className={cn("w-5 h-5 rounded-full flex items-center justify-center border-2", p.enabled ? "bg-black border-black text-white" : "bg-white border-neutral-200 text-transparent")}>
          ...
          <div className="pt-6 border-t border-neutral-200">
            <Button variant="primary" size="md" className="w-full" icon={ShieldCheck}>Update Role Matrix</Button>
            <button className="w-full text-center mt-4 text-[10px] technical-label text-rose-500 font-black italic uppercase hover:underline">Revoke Global Access</button>
          </div>
        </div>
      </div>
    </div>
